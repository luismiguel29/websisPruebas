$(document).ready(function () {
    ...
    $("#idBtnEliminar").click(function () {
        ...
        if (intChecked >= 1) {
            resp = confirm(
                "�Est� realmente seguro de que desea RETIRAR la(s) materia(s) marcada(s)?",
            );
            if (resp == true) {
    ...
    $(".idBtnCambiar").click(function () {
        ...
        var modoCambiar = select.length > 0 ? select.val() : null;
    ...
    $("#idFrmTerminar").submit(function (e) {
        ...
        resp = confirm(
            "¿Está realmente seguro de que desea SALIR del Proceso de Inscripción?",
        );
